 ...
 * component library (see artcollect-design-system.md §4/§5) as plain
 * SVG-DOM constructors, since this stack has no component framework.
 *
 * Per artcollect-design-system.md §7 and §10: these are optional decoration
 * for empty states, dividers, and marketing/landing content only.
 * NEVER import this module from a booking, payment, escrow-confirmation,
 * or dispute page — see pages/checkout.js for the enforced example.
 *
 * All shapes here are purely decorative and always aria-hidden.
 */

const SVG_NS = "http://www.w3.org/2000/svg";

/**
 * A torn-paper divider strip. Color comes from `currentColor` — set it via
 * a CSS `color` rule (e.g. `color: var(--ac-accent)`) on the container.
...
/**
 * A rotated sticky-note backing behind a short line of text — the vanilla
 * equivalent of artcollect.co.ke's <Annotation> component. Only the
 * pre-verified tone pairings below are exposed, so a caller cannot
 * accidentally create an illegible text/backing combination (mirrors the
 * contrast-safety guarantee described in artcollect-design-system.md §5).
 */
