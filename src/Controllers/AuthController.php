 ...
 * /api/v1/auth/* (routes/api.php) for the booking page's fetch() calls.
 * This is this platform's implementation of the shared identity/auth
 * module (see planning/00-portfolio/shared-architecture.md module 1).
 */
final class AuthController
{
    ...
    public function register(Request $request): void
    {
        ...
        $old = ['full_name' => $fullName, 'phone_number' => $phone, 'email' => $email, 'account_type' => $accountType];
        ...
        try {
            ...
            // Customers are Tier 1 trust (see shared-architecture.md's
            // trust-tiering table: "Phone number + national ID number
            // (unverified against registry)") so they can book immediately.
            // Operators stay pending_verification until KYC is approved
            // (see OnboardingController), per this platform's Supply-Side
            // Journey step 3 in user-flows.md.
            ...
            $stmt = $db->prepare(
                'INSERT INTO users (phone_number, email, password_hash, full_name, account_type, status, created_at, updated_at)
                 VALUES (:phone, :email, :hash, :name, :type, :status, NOW(), NOW())'
            );
            $stmt->execute([
                'phone' => $phone,
                'email' => $email,
        ...
        self::redirect($accountType === 'provider' ? '/book' : '/book');
    }
